...
from src.integrations import lyrics_api, art_gallery_api, sound_library_api, social_media_api

class TestCreativeLab(unittest.TestCase):
    ...
    def test_response_generator(self):
        response = response_generator.generate_response("Hello")
        self.assertIn("Here’s what I found:", response)

    def test_context_manager(self):
        cm = context_manager.ContextManager()
        cm.add_entry("User", "Hello")
        self.assertEqual(cm.get_last_response(), "Hello")

    def test_sentiment(self):
        self.assertEqual(sentiment.analyze_sentiment("I love art"), "Positive")

    # --- Productivity ---
    def test_idea_tracker(self):
        idea = idea_tracker.add_idea("New poem idea")
        self.assertIn("New poem idea", idea)

    def test_reminder(self):
        reminder_text = reminder.set_reminder("Finish project")
        self.assertTrue("Finish project" in reminder_text)

    # --- Integrations ---
    def test_lyrics_api(self):
        lyrics = lyrics_api.get_lyrics("Imagine")
        self.assertTrue(isinstance(lyrics, str))
